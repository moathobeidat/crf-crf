...
export default function Home() {
  ...
  const { results, isLoading, error, updateParams, setKeyword, changeSort, setPriceRange } = useSearch({
    keyword: "lego",
    currentPage: 0,
    pageSize: 32,
    sortBy: "relevance",
  ...
  return (
    <>
      <Header
        brand="lego"
        cartCount={Object.values(favorites).filter(Boolean).length}
        wishlistCount={Object.values(favorites).filter(Boolean).length}
      />
      <main className="w-full">
        <div className="max-w-[1450px] mx-auto px-4 sm:px-6 md:px-8 lg:px-[4.0625rem] md:ml-8 lg:ml-[6rem] md:mr-8 lg:mr-[6rem] py-10">
          <h1 className="text-3xl font-light mb-8">LEGO Collection</h1>
          {isLoading ? (
            <ProductGridSkeleton />
          ) : (
